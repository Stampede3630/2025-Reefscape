...
public class LimelightHelpers {
    ...
    private static PoseEstimate[] getBotPoseEstimate(String limelightName, String entryName, boolean isMegaTag2) {
        ...
        TimestampedDoubleArray[] tsQueue = poseEntry.readQueue();
        List<PoseEstimate> poseEstimates = new LinkedList<>();
        for (TimestampedDoubleArray tsValue : tsQueue) {
            double[] poseArray = tsValue.value;
            long timestamp = tsValue.timestamp;

            if (poseArray.length == 0) {
                // Handle the case where no data is available
                return new PoseEstimate[0]; // or some default PoseEstimate
            }
            ...
            if (poseArray.length != expectedTotalVals) {
                ...
                for (int i = 0; i < tagCount; i++) {
                    ...
                    rawFiducials[i] = new RawFiducial(id, txnc, tync, ta, distToCamera, distToRobot, ambiguity, adjustedTimestamp);
                }
            }
            poseEstimates.add(new PoseEstimate(pose, adjustedTimestamp, latency, tagCount, tagSpan, tagDist, tagArea, rawFiducials, isMegaTag2));
        }
    ...
    /**
     * Gets the timestamped queue of raw fiducial/AprilTag detection results from NetworkTables.
     *
    ...
    public static RawFiducial[][] getRawFiducials(String limelightName) {
        var entry = LimelightHelpers.getLimelightDoubleArrayEntry(limelightName, "rawfiducials");
        var rawFiducialQueue = entry.readQueue();
        int valsPerEntry = 7;
        List<RawFiducial[]> rawFiducialsList = new LinkedList<>();
        for (TimestampedDoubleArray rawFiducialValue : rawFiducialQueue) {
            double[] rawFiducialArray = rawFiducialValue.value;
            if (rawFiducialArray.length % valsPerEntry != 0) {
                rawFiducialsList.add(new RawFiducial[0]);
            }
            ...
            for (int i = 0; i < numFiducials; i++) {
                ...
                double timestamp = rawFiducialValue.timestamp * 1.0E-6;
                rawFiducials[i] = new RawFiducial(id, txnc, tync, ta, distToCamera, distToRobot, ambiguity, timestamp);
            }
            rawFiducialsList.add(rawFiducials);
        }

        return rawFiducialsList.toArray(new RawFiducial[0][0]);
    }
    ...
    public static class RawFiducial {
        ...
        public RawFiducial(int id, double txnc, double tync, double ta, double distToCamera, double distToRobot, double ambiguity, double timestamp) {
            this.id = id;
            this.txnc = txnc;
            this.tync = tync;
            this.ta = ta;
        ...
        public boolean equals(Object obj) {
            ...
            return id == other.id &&
                ...
                Double.compare(timestamp, other.timestamp) == 0;
        }
