    ...
    for (int cameraIndex = 0; cameraIndex < io.length; cameraIndex++) {
      ...
      for (var observation : inputs[cameraIndex].txTyObservations) {
        if (!allTxTyObservations.containsKey(observation.tagId())
          ...
          // add this observation to the set if it is a new tag, or if it is closer than the previous camera
          allTxTyObservations.put(observation.tagId(), observation);
        }
      }

      // Loop over pose observations and sort poses into rejects/accept. (This is what's coming from MT1/2)
